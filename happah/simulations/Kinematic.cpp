...
glm::mat4 Kinematic::getMatrix(double t){
    glm::mat4 result(1);
    // Rotation
    result = glm::rotate(result, (float)m_rotateX.evaluateAt(t), glm::vec3(1,0,0));
    result = glm::rotate(result, (float)m_rotateY.evaluateAt(t), glm::vec3(0,1,0));
    result = glm::rotate(result, (float)m_rotateZ.evaluateAt(t), glm::vec3(0,0,1));
    // Translation
    result = glm::translate(result,
                   glm::vec3(m_translateX.evaluateAt(t), m_translateY.evaluateAt(t), m_translateZ.evaluateAt(t)) );
    return result;
}
...
RigidAffineTransformation Kinematic::getRigidAffineTransformation(double t){
	glm::mat4 matrix(1);
    // Rotation
	matrix = glm::rotate(matrix, (float)m_rotateX.evaluateAt(t), glm::vec3(1,0,0));
	matrix = glm::rotate(matrix, (float)m_rotateY.evaluateAt(t), glm::vec3(0,1,0));
	matrix = glm::rotate(matrix, (float)m_rotateZ.evaluateAt(t), glm::vec3(0,0,1));
    // Translation
    return RigidAffineTransformation( glm::mat3(matrix), hpvec3(m_translateX.evaluateAt(t), m_translateY.evaluateAt(t), m_translateZ.evaluateAt(t)) );
}
