...
/**
 * @class The WormGearGrindResult
 * @brief This class bundles all the information needed to display a worm gear
 * grind simulation to a given time.
 */
...
private:
	...
	void computeIntersectingTriangles(hpuint& wormPosZIdx, CircularSimulationResult* simResult, hpmat4x4& gearModelMatrix, hpmat4x4& wormModelMatrix);

	Circle transformCircle(Circle& circle, hpmat4x4 transformation);
	hpvec3 transformPoint(hpvec3& point, hpmat4x4& transformation);
	hpvec3 transformVector(hpvec3& vector, hpmat4x4& transformation);
...
private:
	...
	std::map<
			hpreal,
			WormGearGrindResult> m_precalcResults;
	 ...
	 */
	constexpr static size_t STEP_COUNT = 10;

	constexpr static size_t m_resultAngleSlotCount = 10;
	constexpr static size_t m_resultPosZSlotCount = 30;

};

typedef std::shared_ptr<
		WormGearGrind> WormGearGrind_ptr;
