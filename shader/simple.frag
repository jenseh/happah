...
uniform int isSkipLightingContributionComputation;			
...
void main() {
	if (drawSelectionColors == 0){
		if (isSkipLightingContributionComputation == 0) {				//TODO: do we actually need NO Lighting computation ??
		...
		// add all Contributions
				...
				if (isSelected != 0)
				fragmentColor = fragmentColor+vec4(0.0f,0.5f,0.0f,0.0f);
    	} else {
